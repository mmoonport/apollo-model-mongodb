...
class RelationDirective extends _graphqlTools.SchemaDirectiveVisitor {
  constructor(..._args) {
      ...
      if (input.updateMany) {
        await Promise.all(input.updateMany.map(updateMany => {
          ...
          let {
            ...
            validations,
            arrayFilters
          } = (0, _utils.prepareUpdateDoc)(data);

          if (Object.keys(validations).length !== 0) {
            where = {
              $and: [where, validations]
            };
          }
    ...
    (0, _defineProperty2.default)(this, "_resolveMany", field => async (parent, args, context, info) => {
      ...
      if (!fieldTypeWrap.isAbstract()) {
        selector = await (0, _utils2.applyInputTransform)(context)(args.where, whereType);
      }
    ...
    (0, _defineProperty2.default)(this, "_updateOneQuery", async ({
      collection,
      selector,
      doc,
      context,
  ...
  visitFieldDefinition(field, {
    ...
    let isAbstract = (0, _utils.getDirective)(fieldTypeWrap.realType(), 'abstract'); // if (
    //   !(
...
const createInput = ({
  ...
  if ([INPUT_UPDATE_MANY_RELATION, INPUT_UPDATE_MANY_REQUIRED_RELATION].includes(kind)) {
    ...
    let updateManyType = new _graphql.GraphQLList(inputTypes.get(initialType, updateKind));
    fields.updateMany = {
      name: 'updateMany',
      type: updateManyType,
